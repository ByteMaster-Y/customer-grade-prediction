...
라벨이 없는 유사 데이터(사근 데이터)를 클러스터링 기반으로 라벨링(pseudo-labeling)한 후, 
이를 기존 라벨이 있는 데이터(소융 데이터)와 결합하여 분류 모델(Random Forest)의 성능 향상을 확인하는 실험입니다.
"""
...
# ---------------------------------------------------------
# 3. 클러스터링 적용 (KMeans, k=3)
#    - 소융의 고객 등급이 0~2이므로 클러스터 수도 3으로 설정
# ---------------------------------------------------------
kmeans = KMeans(n_clusters=3, random_state=42)
clusters = kmeans.fit_predict(df_sagun[["연간_구매", "구매_빈도"]])

# 클러스터 결과를 새로운 컬럼으로 추가
df_sagun["클러스터"] = clusters

# ---------------------------------------------------------
# 4. 반품률 결측 보정 (평균 반품률로 채움)
#    - 원래 사근에는 반품률 정보가 없다고 가정
# ---------------------------------------------------------
