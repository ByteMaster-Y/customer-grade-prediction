...
### 문제정의
소융 쇼핑몰은 기존에 연간 구매 금액, 구매 빈도, 반품률 데이터를 수집하여 고객 등급을 관리해왔습니다.
최근 인수한 사근 쇼핑몰은 연간 구매 금액과 구매 빈도만 관리하고 있으며, 반품률과 고객 등급 데이터는 존재하지 않습니다.

현재 상황은 다음과 같습니다.
소융 쇼핑몰 고객 데이터는 특성(3개)과 고객 등급 라벨이 모두 존재하여 지도 학습이 가능합니다.
사근 쇼핑몰 고객 데이터는 일부 특성만 존재하여 비지도 학습만 가능한 상태입니다.

이러한 상황을 고려하여,라벨이 있는 데이터(소융)와 라벨이 없는 데이터(사근)를 모두 활용해고객 등급을 예측하는 모델을 만드는 방법을 제안하고자 합니다.

<img width="1551" height="396" alt="image" src="https://github.com/user-attachments/assets/3014921a-aba7-41c3-a47a-4feb5dd3c0c0" />
